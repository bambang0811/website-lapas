...
const uploadFilesToCloudinary = async (files = []) => {
  ...
  for (const file of files) {
    ...
    const isVideo =
      ...
      /\.(mp4|webm|mov|m4v|avi)$/i.test(file.originalname || "");
    ...
    try {
      ...
      console.log(
        `${isVideo ? "VIDEO" : "IMAGE"} berhasil upload Cloudinary:`,
        uploaded.secure_url,
      );
      ...
      console.error(
        `GAGAL upload ${isVideo ? "VIDEO" : "IMAGE"} ke Cloudinary:`,
        error,
      );

      throw new Error(
        `Upload ${isVideo ? "video" : "gambar"} ke Cloudinary gagal: ${
          error.message
        }`,
      );
    }
...
export async function createBerita(req, res) {
  try {
    ...
    const uploadedVideoUrls =
      await uploadFilesToCloudinary(uploadedVideos);
    ...
    const imageUrl = imageList.length
...
export async function updateBerita(req, res) {
  try {
    ...
    let imageList = normalizeImageList(
      existingRows[0].gambar_url,
      [],
    );

    if (typeof gambar_url !== "undefined") {
      imageList = normalizeImageList(
        gambar_url,
        uploadedImageUrls,
      );
